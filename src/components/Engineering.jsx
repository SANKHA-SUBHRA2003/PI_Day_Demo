...
const dimensions = [
  { num: 'D1', icon: '🏗️', tag: 'Dimension 1', title: 'Data Pool Design & Governance', color: '#0F766E',
  ...
  { num: 'D2', icon: '🔗', tag: 'Dimension 2', title: 'Data Model & OCEL Quality', color: '#7C3AED',
  ...
  { num: 'D3', icon: '⚡', tag: 'Dimension 3', title: 'PQL Efficiency', color: '#1460A8',
  ...
  { num: 'D4', icon: '🧵', tag: 'Dimension 4', title: 'Data Lineage & Auditability', color: '#B45309',
  ...
  { num: 'D5', icon: '💓', tag: 'Dimension 5', title: 'Platform Health', color: '#BE185D',
  ...
  { num: 'D6', icon: '🧹', tag: 'Dimension 6', title: 'Code Quality & Standards', color: '#0E7490',
...
const phases = [
  {
    icon: '🔍', name: 'Platform Audit', color: '#0F766E',
    week: 'Phase 1 · Weeks 1–2',
    desc: 'Run a comprehensive assessment across all six dimensions to establish an honest baseline. The output is a scored Scorecard and a prioritised remediation backlog — not a generic list, but items ranked by impact on AI readiness and platform stability.',
    points: [
  ...
  {
    icon: '🛠️', name: 'Foundation Fixes', color: '#7C3AED',
    week: 'Phase 2 · Weeks 2–4',
    desc: 'Address the highest-impact technical debt items first. These foundational fixes often unlock significant performance gains and data quality improvements before any new features are built on top of them.',
    points: [
  ...
  {
    icon: '🏛️', name: 'Model Quality', color: '#1460A8',
  ...
  {
    icon: '📋', name: 'Governance Layer', color: '#B45309',
    week: 'Phase 4 · Weeks 5–6',
    desc: 'Build the systems and habits that prevent technical debt from accumulating again. Governance is not bureaucracy — it is the set of lightweight, automated guardrails that keep the platform healthy as the team grows.',
    points: [
        ...
        <div className="card-grid-3" style={{ marginBottom: 80 }}>
          {dimensions.map((d, i) => (
            <AnimCard key={i} delay={i * 0.1}>
              <div className="info-card-icon" style={{ background: `${d.color}15`, fontSize: 24 }}>{d.icon}</div>
              <span className="badge" style={{ borderColor: `${d.color}40`, color: d.color, background: `${d.color}10`, marginBottom: 12 }}>
                {d.tag}
              </span>
